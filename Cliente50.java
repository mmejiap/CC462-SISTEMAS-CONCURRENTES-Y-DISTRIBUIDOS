    ...
    void iniciar(){
       new Thread(
            new Runnable() {
                ...
                @Override
                public void run() {
                    //mTcpClient = new TCPClient50("192.168.88.11",
	...
	new Thread(
		new Runnable(){
			...
			public void run(){
				...
				ArrayList<Integer> pesoDeRutasTotales = new ArrayList<Integer>();
					...
					for(int j=i+1;j<paraderos_sort.length;j++){
						if(gNS.encontrarRutaDijkstra(i,j)!=null){
							if(gNS.encontrarRutaDijkstra(i,j).size()>2){
								...
								pesoDeRutasTotales.add(gNS.pesoCamino(i,j));
								gNS.printCamino(i,j);
							}
						}
					}
				...
				//System.out.println("dim PesoRutas->"+pesoRutas.length);	
				System.out.println("tam numRuta->"+numRuta);
				int[][] pesoRutas = new int[numRuta][2];

				for(int a=0;a<pesoDeRutasTotales.size();a++){
					pesoRutas[a][0]=a;
					pesoRutas[a][1]=pesoDeRutasTotales.get(a);
				}
...
/*
	...
	public static void sortbyColumn_v2(int arr[][]){
		int i, j, min_idx;
		for(i=0;i<n-1;i++){
			if(arr[i][1]<arr[min_dx][1])
				min_idx=j;
			int temp1,temp2;
			temp1 = arr[i][0];
			arr[i][0]=arr[min_dx][0];
			arr[min_dx[0]]=temp1;
